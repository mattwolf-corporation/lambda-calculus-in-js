...
/// Observable with stack

// const onChangeWithStack = listeners => value => callback => {
//     listeners = push(listeners)(callback); // callback is a function
//     callback(value, value);
// }


const ObservableLambdaStack = value => {
    let listeners = emptyStack;

    const getValue = () => value;

    const onChangeWithStack = value => callback => {
        listeners = push(listeners)(callback); // callback is a function
        callback(value, value);
    }

    const setValueWithStack = value => newValue => {
        if (value === newValue) return;
        const oldValue = value;
        value = newValue;

        const callbackFunc = (callback, _) => {
            callback(value, oldValue)
        };

        forEach(listeners)(callbackFunc);
    };

    return triple(onChangeWithStack(value))(getValue)(setValueWithStack(value));
};
